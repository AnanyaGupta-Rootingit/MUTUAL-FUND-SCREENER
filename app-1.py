# ==============================================================================
# Mutual Fund Analysis Streamlit App (Optimized, No sklearn)
# ==============================================================================
...
def compute_scheme_metrics(nav_df, ref_date=None):
    """Compute CAGR, volatility, Sharpe, Sortino for each scheme."""
    rf = 0.06
    results = []
    if ref_date is None: 
    ...
    for code in nav_df["scheme_code"].unique():
        ...
        def nav_on_or_before(years):
            ...
            return tmp["nav"].iloc[-1] if not tmp.empty else None
        ...
        if len(rets_1y)>=2:
            ...
            compounded = (1+rets_1y).prod()
            ann_ret = compounded**(252.0/len(rets_1y))-1
...
if metadata is not None and nav_history is not None:
    ...
    if not nav_code or not nav_date or not nav_nav: 
        st.error("NAV must have 'scheme_code','date','nav'")
        st.stop()

    metadata = metadata.rename(columns={meta_code:"scheme_code"})
    if meta_name: 
        metadata = metadata.rename(columns={meta_name:"scheme_name"})
